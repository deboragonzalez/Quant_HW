...
viewed locally. This lesson corresponds to 04_ExploratoryAnalysis/Principles_of_Analytic_Graphics.)
...
| comparison. You'll hear more about this when you study statistical inference (another great course BTW), but evidence for
| a hypothesis is always relative to another competing or alternative hypothesis.
...
| asthma is bigger using the air cleaner.
...
| symptom-free days for most asthmatic children. The plot on the right (using the air cleaner) is generally higher than the
| one on the left (the control group).
...
| Two-variable plots are what you saw in high school algebra.  Remember those x,y plots when you were learning about slopes
| and intercepts and equations of lines? They're valuable, but usually questions are more complicated and require more
| variables.

Sometimes, if you restrict yourself to two variables you'll be misled and draw an incorrect conclusion.

| As you move to the right along the x-axis pollution increases, but the blue line moves downward, indicating fewer deaths.
...
| Simpson's paradox, or the Yule–Simpson effect. Wikipedia (http://en.wikipedia.org/wiki/Simpson%27s_paradox) tells us that
| this "is a paradox in probability and statistics, in which a trend that appears in different groups of data disappears
...
| the relationship between pollution and hospitalization of people with heart disease. As you can see, it's a lot different
| from our previous plots. The solid circles in the center portion indicate point estimates of percentage changes in
| hospitalization rates for different levels of pollution. The lines through the circles indicate confidence intervals
| associated with these estimates. (You'll learn more about confidence intervals in another great course, the one on
...
| (PM2.5), the "annual mean, averaged over 3 years" cannot exceed 12 micro grams per cubic meter. We stored the data from
| the U.S. EPA web site in the data frame pollution. Use the R function head to see the first few entries of pollution.
...
| We see 5 columns of data. The pollution count is in the first column labeled pm25. We'll work mostly with that. The other
| 4 columns are a fips code indicating the state (first 2 digits) and county (last 3 digits) with that count, the
| associated region (east or west), and the longitude and latitude of the area. Now run the R command dim with pollution as
| an argument to see how long the table is.
...
> rug(ppm)
Warning message:
In rug(ppm) : some values will be clipped
...
| Your dedication is inspiring!
  ...
  |===============================================                                                                   |  41%
| Again, we see one data point greater than 18, one between 17 and 18, several between 16 and 17
| and two between 15 and 16, verifying what rug indicated.
...
| So this histogram with more buckets is not nearly as smooth as the preceding one. In fact, it's
| a little too noisy to see the distribution clearly. When you're plotting histograms you might
| have to experiment with the argument breaks to get a good idea of your data's distribution. For
| fun now, rerun the R command rug with the argument ppm.
...
| That's a job well done!
  ...
  |=====================================================                                                             |  47%
| See how rug works with the existing plot? It automatically adjusted its pocket size to that of
...
abline(v = 12, lwd = 2)
 See the vertical line at 12? Not very visible, is it, even though you specified a line width of
| 2? Run abline with the argument v equal to median(ppm), the argument col equal to "magenta", and
| the argument lwd equal to 4.

> abline( v = median(ppm), col = "magenta", lwd = 4)

Better, right? Thicker and more of a contrast in color. This shows that although the median
| (50%) is below the standard, there are a fair number of counties in the U.S that have pollution
...
| what's there. Run the R command table on this column. Use the construct pollution$region. Store
| the result in the variable reg.
...
| pollution data characterized each of the 576 entries as belonging to one of two regions (east
| and west).
...
| specify the formula pm25~region. Call boxplot now with this formula as its argument, data equal
...
| argument set up the margins. Before we plot the histograms let's explore the R command subset
| which, not surprisingly, "returns subsets of vectors, matrices or data frames which meet
| conditions". We'll use subset to pull off the data we want to plot. Call subset now with
| pollution as its first argument and a boolean expression testing region for equality with the
...
| usually x and y. Let's look at a scatterplot showing the relationship between latitude and the
| pm25 data. We'll use plot, a function from R's base plotting package.

| We've seen that we can use a function call as an argument when calling another function. We'll
| do this again when we call plot with the arguments latitude and pm25 which are both from our
| data frame pollution. We'll call plot from inside the R command with which evaluates "an R
| expression in an environment constructed from data". We'll use pollution as the first argument
| to with and the call to plot as the second. This allows us to avoid typing "pollution$" before
| the arguments to plot, so it saves us some typing and adds to your base of R knowledge. Try this
| now.
...
| using with, call plot directly with 3 arguments. The first 2 are pollution$latitude and ppm. The
| third argument, col, we'll use to add color and more information to our plot. Set this argument
| (col) equal to pollution$region and see what happens.
...
| west. Can we figure out which color is east and which west? See that the high (greater than 50)
| and low (less than 25) latitudes are both red. Latitudes indicate distance from the equator, so
| which half of the U.S. (east or west) has counties at the extreme north and south?
...
| Nice work!
  ...
  |=============================================================================================                     |  81%
| As before, use abline to add a horizontal line at 12. Use two additional arguments, lwd equal to
| 2 and lty also equal to 2.
...
| to use different margins. Type the R command par(mfrow = c(1, 2), mar = c(5, 4, 2, 1)) now.
...
| Excellent job!
  ...
  |===================================================================================================               |  87%
| For the first scatterplot, on the left, we'll plot the latitudes and pm25 counts from the west.
| We already pulled out the information for the counties in the east. Let's now get the
| information for the counties from the west. Create the variable west by using the subset command
| with pollution as the first argument and the appropriate boolean as the second.
...
| You are doing so well!
  ...
  |====================================================================================================              |  88%
| Now call plot with three arguments. These are west$latitude (x-axis), west$pm25 (y-axis), and
| the argument main equal to the string "West" (title). Do this now.
...
1: Summarize the data (usually graphically) and highlight any broad features
